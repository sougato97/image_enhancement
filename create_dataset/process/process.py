...
def apply_gains(bayer_images, red_gains, blue_gains):
  ...
  # bayer_images = bayer_images.permute(0, 2, 3, 1) # Permute the image tensor to BxHxWxC format from BxCxHxW format
  bayer_images = bayer_images.permute(1, 2, 0) # Permute the image tensor to HxWxC format from CxHxW format
  ...
  # print(outs.shape)
  outs  = outs.permute(2, 0, 1)  # Re-Permute the tensor back to CxHxW format
  return outs
...
def gamma_compression(images, gamma=2.2):
  ...
  images = images.permute(1, 2, 0) # Permute the image tensor to HxWxC format from CxHxW format
  outs   = torch.clamp(images, min=1e-8) ** (1.0 / gamma)
  outs   = outs.permute(2, 0, 1)  # Re-Permute the tensor back to BxCxHxW format
  return outs


def demosaic_using_opencv(rggb_image ):
  rggb_image = rggb_image.permute(1, 2, 0) # As in pytorch is was reversed
  rggb_image = rggb_image.numpy()

  # Split the RGGB image into channels
  red_channel = rggb_image[:, :, 0]
  green_channel_tl = rggb_image[1::2, 0::2, 1]
  green_channel_br = rggb_image[0::2, 1::2, 1]
  blue_channel = rggb_image[:, :, 2]

  # Create empty channels for missing color values
  green_channel = np.zeros_like(rggb_image[:, :, 1]).astype(np.float32)
  blue_channel_interp = np.zeros_like(rggb_image[:, :, 2]).astype(np.float32)

  # Bilinear interpolation for green channel (top-left)
  green_channel[1::2, 0::2] = green_channel_tl
  green_channel[0::2, 1::2] = (green_channel_tl + green_channel_br) / 2

  # Bilinear interpolation for blue channel
  blue_channel_interp[1::2, 1::2] = blue_channel[1::2, 1::2]
  blue_channel_interp[1::2, 0::2] = (blue_channel[1::2, 1::2] + blue_channel[1::2, :-1:2]) / 2
  blue_channel_interp[0::2, 1::2] = (blue_channel[1::2, 1::2] + blue_channel[:-1:2, 1::2]) / 2
  blue_channel_interp[0::2, 0::2] = (blue_channel[1::2, 1::2] + blue_channel[:-1:2, :-1:2] + blue_channel[1::2, :-1:2] + blue_channel[:-1:2, 1::2]) / 4

  # Merge the channels to form the RGB image
  rgb_image = cv2.merge([red_channel.astype(np.uint8), green_channel.astype(np.uint8), blue_channel_interp.astype(np.uint8)])

  # Now convert the image back to tensor 
...
def process(bayer_image, red_gains, blue_gains, cam2rgbs):
  """Processes a batch of Bayer RGGB image into sRGB image."""
  # White balance.
  bayer_image = apply_gains(bayer_image, red_gains, blue_gains)
  # Demosaic.
  ...
  # Color correction.
  image = apply_ccm(image, cam2rgbs)
  # Gamma compression.
