...
def convert(rgb_image):
  """Convert the BGR image to an RGGB mosaic."""
  # Resize the image to half its size
  resized_image = cv2.resize(rgb_image, (0, 0), fx=0.5, fy=0.5)

  # Split the resized image into channels
  b, g, r = cv2.split(resized_image)

  # Create an RGGB pattern image using bilinear interpolation
  rggb_image = np.zeros_like(resized_image)
  rggb_image[..., 0] = r  # Red channel
  rggb_image[1::2, 0::2, 1] = g[1::2, 0::2]  # Green channel (top-left)
  rggb_image[0::2, 1::2, 1] = g[0::2, 1::2]  # Green channel (bottom-right)
  rggb_image[..., 2] = b  # Blue channel

  # Upscale the RGGB pattern image to the original size
  upscaled_image = cv2.resize(rggb_image, (rgb_image.shape[1], rgb_image.shape[0]))
  return upscaled_image


def mosaic_using_opencv(image):
  """Extracts RGGB Bayer planes from an RGB image."""
  image = image.permute(1, 2, 0) # As in pytorch is was reversed
  img_opencv = cv2.cvtColor(image.numpy(), cv2.COLOR_RGB2BGR)

  # Convert the BGR image to an RGGB mosaic using OpenCV with bilinear interpolation
  rggb = convert(img_opencv)

  # Now convert the image back to tensor 
  transform = transforms.ToTensor()

  # Convert the image to tensor
  tensor = transform(rggb)
...
def add_noise(image, shot_noise=0.000001, read_noise=0.0005):
...
def gaussian_noise(image):
  ...
  img = image.cpu().numpy() # make sure tensor is on cpu
  img = (img * 255).astype(np.uint8) # convert to uint8
  img = np.transpose(img, (1, 2, 0)) # change the order of channels to HxWxC from CxHxW 
  ...
  transform = transforms.ToTensor()
  # convert it back to a tensor, order wil be reversed to CxHxW format automatically
